#!/usr/bin/env python3
"""Quick 16-check smoke test (not part of the 72)."""
...
from kafka_mcp.approval import mint, mint_expired
...
def main() -> int:
    ...
    r = s.handle(
        {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "create_topic", "arguments": {"name": "agent.smoke"}}},
        {},
    )
    expect("create in-scope topic", "result" in r)

    r = s.handle(
        {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "produce_message", "arguments": {"topic": "agent.smoke", "value": "hi"}}},
        {},
    )
    ...
    r = s.handle(
        {
            ...
            "params": {
                "name": "delete_topic",
                "arguments": {"name": "prod.ledger", "_approval_token": mint(secret, "delete_topic")},
            },
        },
        {},
    )
    ...
    r = s.handle(
        {
            ...
            "id": 1,
            "method": "tools/call",
            "params": {"name": "consume_messages", "arguments": {"topic": "agent.pii", "maxMessages": 5}},
        },
        {},
    )
    val = str(((r.get("result") or {}).get("records") or [{}])[0].get("value"))
    ...
    r = s.handle(
        {
            ...
            "params": {
                "name": "delete_topic",
                "arguments": {"name": "agent.tainted", "_approval_token": mint(secret, "delete_topic")},
            },
        },
        sess,
    )
    ...
    s2 = KafkaMcpServer(Config(readonly=True))
    r = s2.handle(
        {
            "jsonrpc": "2.0",
            "id": 1,
    ...
    s3 = KafkaMcpServer(Config(tools_denied=["list_topics"]))
    r = s3.handle(
        {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "list_topics", "arguments": {}}},
        {},
    )
    ...
    s4 = KafkaMcpServer(Config(rate_requests_per_second=1, rate_admin_requests_per_second=1))
    codes = []
    for _ in range(5):
        rr = s4.handle(
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "list_topics", "arguments": {}}},
    ...
    s5 = KafkaMcpServer(Config(policy_engine=lambda *a, **k: False))
    r = s5.handle(
        {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "list_topics", "arguments": {}}},
        {},
    )
    ...
    s6 = KafkaMcpServer(Config(policy_engine=boom))
    r = s6.handle(
        {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "list_topics", "arguments": {}}},
        {},
    )
    ...
    s7 = KafkaMcpServer(Config())
    s7.backend._inject_dependency_failure(True)
    codes = []
    for _ in range(4):
        rr = s7.handle(
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "describe_cluster", "arguments": {}}},
            {},
        )
        codes.append((rr.get("error") or {}).get("code"))
    expect("dependency failure -32043", -32043 in codes)

    s8 = KafkaMcpServer(Config())
    s8.handle(
        {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "list_topics", "arguments": {}}},
